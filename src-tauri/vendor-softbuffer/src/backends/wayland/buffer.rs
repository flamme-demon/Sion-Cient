...
impl WaylandBuffer {
    pub fn new(shm: &wl_shm::WlShm, width: i32, height: i32, qh: &QueueHandle<State>) -> Self {
        ...
        let buffer = pool.create_buffer(
            ...
            height,
            width * 4,
            // Sion patch: Argb8888 instead of Xrgb8888 so the alpha channel
            // reaches the Wayland compositor (otherwise transparent windows
            // appear opaque-black on KDE/GNOME). Hardcoded since softbuffer
            // 0.4 doesn't expose a per-surface format toggle.
