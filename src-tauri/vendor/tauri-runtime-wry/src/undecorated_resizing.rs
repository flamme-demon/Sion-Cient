  ...
  pub fn attach_resize_handler(webview: &wry::WebView) {
    ...
    webview.connect_button_press_event(
      move |webview: &webkit2gtk::WebView, event: &gtk::gdk::EventButton| {
        if event.button() == 1 {
          // The webview may be nested in additional GTK containers (for
          // example a GtkOverlay used for native video compositing). Resolve
          // the actual toplevel instead of assuming WebView -> GtkBox ->
          // GtkWindow.
    ...
    webview.connect_touch_event(
      move |webview: &webkit2gtk::WebView, event: &gtk::gdk::Event| {
        // Keep touch resizing working when consumers add GTK wrappers around
        // the webview, and never panic across the GTK FFI callback boundary.
        if let Some(window) = webview
          .ancestor(gtk::Window::static_type())
          .and_then(|widget| widget.downcast::<gtk::Window>().ok())
        {
