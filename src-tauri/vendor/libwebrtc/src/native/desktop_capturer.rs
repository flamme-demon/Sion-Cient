...
impl DesktopCapturer {
    ...
    {
        // Sous Wayland, le portail PipeWire délivre ses réponses D-Bus sur le
        // contexte GLib thread-default. La version d'origine utilisait le
        // contexte global et lançait sa boucle sur un thread séparé ; avec une
        // UI GTK/Tauri (WRY) ce contexte global appartient à GTK et les
        // callbacks du portail ne sont jamais dispatchés : le sélecteur
        // d'écran s'affiche, la sélection aboutit, mais aucune frame n'arrive
        // (erreurs temporaires en boucle). On crée donc un contexte privé,
        // poussé en thread-default le temps de créer le proxy D-Bus (Start),
        // puis itéré sur son propre thread.
        #[cfg(all(any(target_os = "linux", target_os = "freebsd"), feature = "glib-main-loop"))]
        if std::env::var("WAYLAND_DISPLAY").is_ok() {
            ...
            // Aucun thread ne possède encore le contexte : l'acquisition
            // réussit forcément, et le proxy D-Bus créé ici reste attaché à
            // ce contexte privé.
            if let Err(e) = context.with_thread_default(|| {
                pin_handle.start(Box::new(callback_wrapper));
            }) {
                log::error!("[Sion][capture] contexte GLib thread-default indisponible: {e}");
            }
