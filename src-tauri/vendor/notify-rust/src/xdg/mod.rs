...
/// `zbus::block_on` (feature `tokio`) construit un runtime Tokio et panique
/// quand il est appelé depuis un thread qui exécute déjà un runtime — ce qui
/// est le cas de `tauri::async_runtime` via tauri-plugin-notification. On
/// isole donc l'appel bloquant sur un thread std dédié, sans runtime actif.
#[cfg(feature = "zbus")]
fn block_on_detached<F>(future: F) -> F::Output
where
    F: std::future::Future + Send,
    F::Output: Send,
{
    std::thread::scope(|scope| scope.spawn(move || block_on(future)).join().unwrap())
}

#[cfg(all(feature = "zbus", not(feature = "dbus")))]
pub(crate) fn show_notification(notification: &Notification) -> Result<NotificationHandle> {
    block_on_detached(zbus_rs::connect_and_send_notification(notification)).map(Into::into)
}
