...
fn main() {
    ...
    let sion_audio = env::var_os("CARGO_FEATURE_SION_AUDIO").is_some();
    if sion_audio { rust_files.push("src/sion_audio.rs"); }
    let mut builder = cxx_build::bridges(rust_files);
    if sion_audio {
        builder.define("SION_NATIVE_AUDIO", None);
        builder.file("src/sion_audio.cpp");
    }
