...
rust::String AudioDeviceController::playout_device_guid(uint16_t index) const {
  ...
  char guid[webrtc::kAdmMaxGuidSize] = {0};
  adm_proxy_->PlayoutDeviceName(index, name, guid);
  // PulseAudio's ADM can enumerate every endpoint while leaving every GUID
  // empty. Keep those devices selectable by exposing the ADM index as an
  // opaque fallback ID. The corresponding setters below resolve it back to
  // the same index.
  if (guid[0] == '\0') {
    return rust::String("sion-adm-index:" + std::to_string(index));
  }
...
bool AudioDeviceController::set_playout_device_by_guid(rust::String guid) const {
  ...
  const std::string requested(guid.c_str());
  ...
  for (int16_t i = 0; i < count; i++) {
    ...
    if (adm_proxy_->PlayoutDeviceName(i, name, device_guid) == 0) {
      const std::string fallback = "sion-adm-index:" + std::to_string(i);
      if (requested == std::string(device_guid) ||
          (device_guid[0] == '\0' && requested == fallback)) {
